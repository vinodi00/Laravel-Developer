...
<div class="max-w-xl mx-auto py-8">
    ...
    <form method="POST" action="{{ route('tasks.update', $task) }}" class="space-y-4 bg-white p-6 rounded shadow">
        @csrf
        @method('PUT')

        <div>
            <label class="block text-sm font-medium mb-1">Title</label>
            <input name="title" value="{{ old('title', $task->title) }}" class="border rounded w-full px-3 py-2" required>
